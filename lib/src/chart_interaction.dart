...
class GChartInteractionHandler with Diagnosticable {
  ...
  MouseCursor _mouseCursor({required Offset position}) {
    ...
    for (int p = 0; p < _chart.panels.length; p++) {
      ...
      if (panel.resizable &&
          ...
          _chart.nextVisiblePanel(startIndex: p + 1)?.resizable == true &&
    ...
    for (int n = 0; n < _chart.panels.length; n++) {
      ...
      if (area.contains(position)) {
        ...
        if (_chart.pointerScrollMode == GPointerScrollMode.move) {
          ...
          final centerPoint = pointViewPort
          ...
          _hookScaleUpdate =
              ({
                ...
                if (axis.scaleMode == GAxisScaleMode.zoom ||
                    (scale > 0 && scale != 1.0)) {
                  double scaleRatio =
                      (axisArea.right - position.dx) /
                      (axisArea.right - start.dx);
                  ...
                  double scaleRatio = min(
                    max(
                      (axisArea.bottom - start.dy) /
                          (axisArea.bottom -
                              min(position.dy, axisArea.bottom - 1)),
                      0.01,
                    ),
                    100,
                  );
  ...
  GGraph? _tryScalingGraph(GPanel panel, Offset start, int pointerCount) {
    ...
    _hookScaleEnd = (int pointerCount, double scaleVelocity, Velocity? velocity) {
      _hookScaleEnd = null;
      _chart.mouseCursor.value = SystemMouseCursors.precise;
      bool momentum =
          velocity != null &&
      ...
      if (momentum) {
        ...
        final startMin =
            _chart.dataSource.firstPoint - pointViewPort.pointRangeSize / 2;
        final endMax =
            _chart.dataSource.lastPoint + pointViewPort.pointRangeSize / 2;
